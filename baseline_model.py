...
class SVMClassifier:
    ...
    def train_with_validation(self, X_train, y_train, X_val, y_val,
        ...
        # 当 n_features > n_samples 时使用dual=True
...
def run_pipeline():
    ...
    for config in hog_configs:
        ...
        hog_extractor = HOGFeatureExtractor(**config['params'])
        ...
        # 3. 创建分类器
        classifier = SVMClassifier(hog_extractor=hog_extractor)
    ...
    if best_classifier is not None:
        ...
        if choice == 'y':
            print("\n使用全量数据训练最终模型...")
            final_hog_extractor = HOGFeatureExtractor(**best_classifier.hog_extractor.__dict__)
            final_classifier = SVMClassifier(hog_extractor=final_hog_extractor)
